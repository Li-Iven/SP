...
int tSM::p01(){ //       S -> PROG
	...
	for(tGlobal::iterator it=globals.begin();
		...
		std::cout << it->first << " " << it->second.line << " p\n";
	}//for...
	std::cout << "\n";
	...
	switch(scope){
		case 2:
			if(locals.count(name)) {
				ferror_message+=
				"Error[18-1] in line "+ S1->line +": The procedure call identifier '"
				+name+
		...
		case 1:
			if(params.count(name)) {
				ferror_message+=
				"Error[18-2] in line "+ S1->line +": The procedure call identifier '"
				+name+
		...
		if(ref.test(DEFINED)){
			...
			if(ref.types != S1->types){
				ferror_message+=
				"Error[18-1] in line "+ S1->line +": The types of the call parameters and the definition of the procedure '"
				+name+
				"' do not match!\n";
			...
			if(ref.arity != S1->count){
				ferror_message+=
				"Error[18-2] in line "+ S1->line +": The arities of the call and the definition of the procedure '"
				+name+
				"' do not match!\n";
...
int tSM::p68(){ //     DEF -> PROC
	...
	if(ref.test(USED)){
		...
		if(ref.arity != S1->count){
			ferror_message+=
			"Error[68-2] in line "+ S1->line +": The arities of the call and the definition of the procedure '"
			+name+
			"' do not match!\n";
			// Арности вызова и определения процедуры 'f' не совпадают!
			// The arities of the call and the definition of the procedure 'f' do not match!
